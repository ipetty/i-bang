...
public class PathUtils {
	...
	public static String getDownloadDir() {
		File file = new File(createAppDir() + File.separator + DOWNLOAD_DIR + File.separator);
		if (!file.exists()) {
			file.mkdirs();
		}
		return file.getAbsolutePath() + File.separator;
	...
	public static String getExternalStorageDirectory() {
		if (existExternalStorageDirectory()) {
			...
			return null;// throw new AppException("请插入存储卡");
		}
	}
	...
	public static String getPathByUriFromFile(Uri uri, Context paramContext) {
		...
		if ((!StringUtils.isEmpty(uri.toString())) && (uri.toString().contains("file://"))) {
			...
			if ((!StringUtils.isEmpty(sdPath)) && (sdPath.contains("/mnt")) && (!StringUtils.isEmpty(path))
					&& (!path.startsWith("/mnt"))) {
				path = "/mnt" + path;
			}
			return path;
