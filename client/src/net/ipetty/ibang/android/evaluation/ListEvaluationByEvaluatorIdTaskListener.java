 ...
 * @date 2014年11月4日
 */
public class ListEvaluationByEvaluatorIdTaskListener extends DefaultTaskListener<List<EvaluationVO>> {
	...
	public ListEvaluationByEvaluatorIdTaskListener(Activity activity, EvaluationAdapter adapter,
	...
	public void onSuccess(List<EvaluationVO> evaluations) {
		Log.d(TAG, "list evaluations by evaluator id success");
		if (isRefresh) {
		...
		if (activity instanceof MyEvaluationActivity) {
			((MyEvaluationActivity) activity).loadMoreForResult_from_me(evaluations);
		}
	}
