package net.ipetty.ibang.cache.interceptor;
...
import net.ipetty.ibang.util.AopUtils;
...
public class CachesInterceptor {
	...
	private final String LOAD_FROM_CACHE = "@annotation(net.ipetty.ibang.cache.annotation.LoadFromCache)";
	private final String UPDATE_TO_CACHE = "@annotation(net.ipetty.ibang.cache.annotation.UpdateToCache)";
	private final String UPDATES_TO_CACHE = "@annotation(net.ipetty.ibang.cache.annotation.UpdatesToCache)";
	...
	public <K, V> V get(ProceedingJoinPoint call) throws Throwable {
		...
		try {
			...
			logger.debug("Cache.get('{}', {}) = {}", mapName, key, value);

			if (value == null) {
				value = (V) call.proceed();
				if (value != null) {
					Caches.set(mapName, key, value);
					logger.debug("Cache.set('{}', {}, {});", mapName, key, value);
				} else {
					Caches.delete(mapName, key);
					logger.debug("Cache.delete('{}', {});", mapName, key);
				}
			}
